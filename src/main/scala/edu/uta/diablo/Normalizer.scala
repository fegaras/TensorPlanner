...
*/
      ...
      case reduce(m,Seq(Nil))
        => Seq(Nil)
      case MethodCall(sc,"parallelize",flatMap(f,b)::rest)
        => flatMap(f,MethodCall(sc,"parallelize",b::rest))
      ...
      case Nth(Tuple(es),n)
        => normalize(es(n-1))
      case Nth(Let(p,v,b),n)
        => normalize(Let(p,v,Nth(b,n)))
      case Nth(IfE(p,x,y),n)
        => normalize(IfE(p,Nth(x,n),Nth(y,n)))
