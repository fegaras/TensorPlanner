...
abstract class CodeGeneration {
  ...
  def codeGen ( e: Expr, env: Environment ): c.Tree = try {
    e match {
      ...
      case Call("reduceByKey",List(x,Lambda(p,b)))
        => val (tp,xc) = typedCode(x,env)
           val pc = code(p)
           val nx = TermName(c.freshName("_x"))
           val ny = TermName(c.freshName("_y"))
           tp match {
             case tq"($kt,$et)"
               => val bc = codeGen(b,add(p,tq"($et,$et)",env))
                  q"reduceByKey($xc,($nx:$et,$ny:$et) => ($nx,$ny) match { case $pc => $bc })"
             case _ => throw new Error("Wrong reduceByKey: "+e)
           }
      ...
      case Lambda(p,b)
        => val tpt = tq""  // empty type
           def patf ( p: Pattern ): c.Tree
             = p match {
                 case VarPat(v)
                   => if (cached_env.contains(v))
                        Type2Tree(cached_env(v))
                      else tpt
                 case TuplePat(ps)
                   => val pc = ps.map(patf)
                      tq"(..$pc)"
               }
           def pate ( p: Pattern, env: Environment ): Environment
             = p match {
                 case VarPat(v)
                   => val tpc = if (cached_env.contains(v))
                                  Type2Tree(cached_env(v))
                                else tpt
                      add(p,tpc,env)
                 case TuplePat(ps)
                   => ps.foldLeft[Environment](env){ case (r,p) => pate(p,r) }
               }
           val nv = newvar
           val nvc = TermName(nv)
           val pc = code(p)
