...
object Typechecker {
    ...
    var cached_env: Environment = Map()
    ...
    def typecheck ( e: Expr, env: Environment ): Type
      = if (e.tpe != null)
          ...
          case MethodCall(u,"reduceByKey",Lambda(p@TuplePat(List(VarPat(x),VarPat(y))),b)::_)
            => val tp = typecheck(u,env)
               val TupleType(List(_,etp)) = elemType(tp)
                 ...
                 = p match {
                     case VarPat(v)
                       => if (cached_env.contains(v))
                            cached_env(v)
                          else TypeParameter(newvar)
                     case TuplePat(ps)
                       => TupleType(ps.map(ptype))
                     case _ => TypeParameter(newvar)
                   }
               val tp = ptype(p)
                  ...
                  case ((_,r),x@VarDecl(v,at,u))
                    => val tp = elemType(typecheck(u,r))  // type of u is concrete
                       if (!typeMatch(tp,at))
                         throw new Error("Incompatible value in variable declaration: "
                                         +x+"\n(expected "+at+" found "+tp+")")
                       val rtp = if (useStorageTypes && tp != AnyType()) tp else at
                       global_env = global_env+(v->rtp)
                       (rtp,r + (v->rtp))
